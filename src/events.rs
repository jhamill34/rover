use std::{sync::{Arc, Mutex}, io, time::Duration};

use crossterm::event::{poll, self, Event, KeyEvent, KeyCode, KeyModifiers};
use json_pointer::JsonPointer;
use redux_rs::{Store, Reducer};
use tui::backend::Backend;

use crate::{state::{State, Page}, action::Action, lifecycle::ApplicationLifecycle, util::{editor, save_doc}};

pub async fn event_listener<R, B>(
    store: Store<State, Action, R>,
    lifecycle: Arc<Mutex<ApplicationLifecycle<B>>>,
) -> anyhow::Result<()>
    where R: Reducer<State, Action> + Send + Sync + 'static,
    ...
    loop {
        if poll(Duration::from_millis(100))? {
            ...
            if let Event::Resize(height, width) = read_event {
                ...
                store.select(move |state: &State| -> anyhow::Result<()> {
                    ...
                    if let Event::Key(key) = read_event {
                        ...
                        match key {
                            ...
                            KeyEvent { code: KeyCode::Char('l'), .. } => {
                                let children = store.select(|state: &State| {
                                    state.nav_state
                                        .current
                                        .options
                            ...
                            KeyEvent { code: KeyCode::Char('e'), .. } => {
                                ...
                                let existing_value = store.select(|state: &State| {
                                    state.nav_state
                                        .current
                                        .options
                                        .get(state.nav_state.current.selected)
                            ...
                            KeyEvent { code: KeyCode::Char('/'), .. } => {
                                store.dispatch(Action::SetCurrentPage { page: Page::Search }).await
                            },
                        ...
                        match key {
                            KeyEvent { code: KeyCode::Char(ch), modifiers: KeyModifiers::SHIFT, ..  } |
                                KeyEvent { code: KeyCode::Char(ch), modifiers: KeyModifiers::NONE, ..  } => {
                                    let mut current = store.select(|state: &State| state.search_state.value.clone()).await;
                                    current.push(ch);
                                    store.dispatch(Action::SearchSetValue { value: current }).await;
                                },
                                ...
                                KeyEvent { code: KeyCode::Enter, .. } => {
                                    store.dispatch(Action::SearchSetValue { value: String::new() }).await;
