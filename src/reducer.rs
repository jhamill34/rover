...
use crate::{state::{State, index::DocIndex, SearchState, Step}, action::Action};
...
pub fn reducer(mut state: State, action: Action) -> State {
    match action {
        ...
        Action::NavSelect => {
            if let Some(selected_path) = state.nav_state.current.options.get(state.nav_state.current.selected) {
        ...
        Action::DocumentReplaceCurrent { value } => {
            if let Some(path) =  state.nav_state.current.options.get(state.nav_state.current.selected) {
                let existing = path.parse::<JsonPointer<_, _>>().ok()
