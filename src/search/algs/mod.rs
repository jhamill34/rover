...
use super::{parser::{Pattern, Operation}, PatternCache};
...
///
pub fn score(pattern: &Pattern, cache: &mut PatternCache, deref_cache: &mut PatternCache, doc: &Value) -> HashMap<String, i16> {
    match pattern {
        &Pattern::Group { ref op, ref left, ref right } => {
            let left_score = score(left, cache, deref_cache, doc);
            let right_score = score(right, cache, deref_cache, doc);

            match op {
                &Operation::And => {
                    left_score.iter().filter_map(|(key, left_score)| {
        ...
        &Pattern::Fuzzy(ref pattern) => {
            ...
            let result: HashMap<String, i16> = existing.keys().filter_map(|key| {
        ...
        &Pattern::Exact(ref pattern) => {
            ...
            let result: HashMap<String, i16> = existing.keys().filter_map(|x| {
        ...
        &Pattern::Prefix(ref pattern) => {
            ...
            let result: HashMap<String, i16> = existing.keys().filter_map(|x| {
                let potential_score = (pattern.len() * 16) as i16;
                x.starts_with(pattern).then_some((x.clone(), potential_score))
            }).collect();
        ...
        &Pattern::Deref(ref pattern) => { 
            score_deref(pattern, deref_cache, doc)
        },
    }
}

///
fn score_deref(pattern: &Pattern, deref_cache: &mut PatternCache, doc: &Value) -> HashMap<String, i16> {
    match pattern {
        &Pattern::Group { ref op, ref left, ref right } => {
                ...
                key.parse::<ValuePointer>().ok()
                    ...
                    .and_then(|node| {
                        match node {
                            ...
                            &Value::Number(ref value) => Some(fuzzy.score(&value.to_string(), pattern)),
                            &Value::Array(ref arr) => {
                                let mut score = 0;
                                for i in 0..arr.len() {
                ...
                key.parse::<ValuePointer>().ok()
                    ...
                    .and_then(|node| {
                        match node {
                            ...
                            &Value::Bool(value) => {
                                let potential_score = (pattern.len() * 16) as i16;
                                value.to_string().contains(pattern).then_some(potential_score)
                            },
                ...
                key.parse::<ValuePointer>().ok()
                    ...
                    .and_then(|node| {
                        match node {
                            ...
                            &Value::Bool(value) => {
                                let potential_score = (pattern.len() * 16) as i16;
                                value.to_string().starts_with(pattern).then_some(potential_score)
                            },
                            &Value::Number(ref value) => {
                                let potential_score = (pattern.len() * 16) as i16;
                                value.to_string().starts_with(pattern).then_some(potential_score)
                ...
                key.parse::<ValuePointer>().ok()
                    ...
                    .and_then(|node| {
                        match node {
                            ...
                            &Value::Bool(value) => {
                                let potential_score = (pattern.len() * 16) as i16;
                                value.to_string().ends_with(pattern).then_some(potential_score)
                            },
                            &Value::Number(ref value) => {
                                let potential_score = (pattern.len() * 16) as i16;
                                value.to_string().ends_with(pattern).then_some(potential_score)
